...
export class EcsRasaStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props: EcsRasaProps) {
    ...
    const customerSuffix = `-${props.customerName}-${props.projectId}`
    const rasarepo = ecr.Repository.fromRepositoryName(this, `${prefix}repository-rasa`, `${props.envName}-rasa${customerSuffix}`);
    const actionsrepo = ecr.Repository.fromRepositoryName(this, `${prefix}repository-actions`, `${props.envName}-actions${customerSuffix}`);
    ...
    const rasatd = new ecs.TaskDefinition(this, `${prefix}taskdefinition-rasa${customerSuffix}`, {
      cpu: '2048',
      memoryMiB: '4096',
    ...
    rasatd.addVolume({
      name: `rasavolume${customerSuffix}`,
    });

    rasatd.addContainer(`${prefix}container-rasa${customerSuffix}`, {
      image: ecs.ContainerImage.fromEcrRepository(rasarepo),
      containerName: `rasa${customerSuffix}`,
      portMappings: [{
        hostPort: props.port,
      ...
      logging: ecs.LogDriver.awsLogs({
        streamPrefix: `${prefix}rasa${customerSuffix}`,
        logRetention: RetentionDays.ONE_DAY
      })
    }).addMountPoints(
      {
        containerPath: '/app/models',
        sourceVolume: `rasavolume${customerSuffix}`,
        readOnly: false
      }
    );
...
/*     rasatd.addContainer(`${prefix}actions`, {
    ...
    const rasaservice = new ecs.FargateService(this, `${prefix}service-rasa${customerSuffix}`, {
      cluster: props.baseCluster,
      taskDefinition: rasatd,
      securityGroups: [rasasg],
      cloudMapOptions: {
        name: `rasa${customerSuffix}`
      }
    });

    const listener = new elbv2.ApplicationListener(this, `${prefix}listener-rasa${customerSuffix}`, {
    ...
    const tg = new elbv2.ApplicationTargetGroup(this, `${prefix}targetgroup-rasa${customerSuffix}`, {
      targets: [rasaservice],
    ...
    listener.addTargetGroups(`${prefix}targetgroupadd-rasa${customerSuffix}`, {
      targetGroups: [tg]
    });
