...
class GraphQLNaturalLanguageGenerator(NaturalLanguageGenerator):
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        try:
            lang_detect_result = fasttext_lang_detect.predict(
                tracker.latest_message.text)
            # check that detect probability over 0.2 and message not init get started msg
            if lang_detect_result[1][0] >= 0.2 and tracker.latest_message.text != "/get_started":
                detected_language = lang_detect_result[0][0].split("__label__")[
                    1]
            else:
                detected_language = None
        except Exception as e:
            logger.error(
                f"Fasttext lang detect returned error: {e}"
            )
            detected_language = None
        ...
        try:
            if "graphql" in self.nlg_endpoint.url:
                ...
                response = self.get_graphql_response(body)
                # if response text starts with utter, response msg hasn't been spesified for that language
                # get response for the original or fallback language instead
                if response["text"].startswith("utter"):
                    body = nlg_request_format(
                        template_name,
                        tracker,
                        output_channel,
                ...
                # legacy route, use first message in seq
                response = response[0]
    ...
    def get_graphql_response(self, body: dict) -> dict:
